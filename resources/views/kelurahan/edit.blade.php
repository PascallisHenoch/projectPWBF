...
@section('container')
	...
	<form action="/kelurahan/update/{{ $kelurahan->id }}" method="post">
			...
			<select class="form-control" id="id_kelurahan" name="id_kelurahan">
				@if($kecamatan->count())
					@foreach($kecamatan as $v)
						<option value="{{ $v->id }}" {{ $kelurahan->id_kelurahan == $v->id ? 'selected="selected"' : '' }}>{{ $v->nama_kecamatan }}</option>
					@endforeach
				@endif
			</select>
		</div>
		<div class="form-group">
			...
			<input type="text" class="form-control" id="nama_kelurahan" name="nama_kelurahan" placeholder="Nama Kelurahan" value="{{ $kelurahan->nama_kelurahan }}">
		</div>
